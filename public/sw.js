...
//
// Strategy: precache the shell, then for same-origin GETs serve cache-first with
// a background refresh (fast + eventually fresh); navigations are network-first
// with an offline fallback to the cached app shell. Cross-origin requests
// (Google sign-in, Drive sync) are left untouched so nothing is intercepted or
// cached that shouldn't be.

const VERSION = 'bmbr-v1';
const BASE = new URL('./', self.location).pathname; // '/' in dev, '/BlackMirrorBikeRide/' on Pages
const SHELL = [
  BASE,
  BASE + 'index.html',
